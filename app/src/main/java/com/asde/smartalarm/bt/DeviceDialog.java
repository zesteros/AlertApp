                ...
                while (tryToConnect[0]) {
                    if (System.currentTimeMillis() - start == global.getTimeout()) {//timeout 20 seg default
                        ...
                        ((MainActivity)context).runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                showToast("Lo sentimos, no se puede conectar a la alarma.");
                                GlobalVariable.getInstance().getConnectButton().setText("Conectar");
                    ...
                    ((MainActivity)context).runOnUiThread(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            GlobalVariable.getInstance().getConnectButton().setText("Conectando...");
                        }
                    });
                    if (global.isConnected()) {
                        ((MainActivity)context).runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                GlobalVariable.getInstance().getConnectButton().setText("Desconectar");
                                GlobalVariable.getInstance()
                                        .getConnectButton()
                                        .setBackground(((MainActivity)context)
                                                .getResources()
                                                .getDrawable(R.drawable.round_button_disconnect));
                                ((MainActivity)context).showToast("Conexión exitosa");
                            }
                        });
